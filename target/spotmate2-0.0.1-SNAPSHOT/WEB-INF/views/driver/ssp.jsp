...
<body>
	<div class="map_wrap">
		<div id="map"
			style="width: 1200px; height: 720px; position: relative; overflow: hidden;"></div>
	...
	<script>
		...
		function setParentText() {
			var i = $("#no").val();
			if ($("#title").val() == "") {
				opener.$("#s-addr"+i).val($("#addr").val());
				opener.$(".hide-s-addr"+i).val($("#addr").val());
			} else {
				opener.$("#s-addr"+i).val($("#title").val());
				opener.$(".hide-s-addr"+i).val($("#title").val());
			}
			opener.$(".s-lat"+i).val($("#lat").val());
			opener.$(".s-lng"+i).val($("#lng").val());
			window.close();
        }
		// 키워드 검색을 요청하는 함수입니다
		...
		function displayPlaces(places) {
			...
			for (var i = 0; i < places.length; i++) {
				...
				})(places[i].place_name, places[i].address_name, placePosition,
						i);
